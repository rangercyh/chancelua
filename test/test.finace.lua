...
tip = 'timezone() returns a timezone'
Helper.times_f(function()
    local timezone = chance:timezone()
    assert(type(timezone.name) == "string", tip)
    assert(string.len(timezone.abbr) < 6, tip)
    assert(type(timezone.offset) == "number", tip)
